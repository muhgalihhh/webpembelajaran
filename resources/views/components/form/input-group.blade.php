...
<div>
    ...
    {{-- Gunakan AlpineJS untuk mengelola state show/hide password --}}
    <div x-data="{ show: false }"
        class="flex items-center px-4 py-2 border border-gray-700 rounded-md shadow-sm bg-gray-50">
        @if ($icon)
        ...
        <input {{-- Bind tipe input ke state 'show' di AlpineJS --}} :type="show ? 'text' : '{{ $type }}'" id="{{ $id }}"
            placeholder="{{ $placeholder }}" wire:model.live="{{ $wireModel }}"
        ...
        @if ($passwordToggle)
            {{-- Ikon ini sekarang akan mengubah state 'show' saat diklik --}}
            <span class="ml-3 text-gray-500 cursor-pointer" @click="show = !show">
                {{-- Ikon berubah secara dinamis berdasarkan state 'show' --}}
